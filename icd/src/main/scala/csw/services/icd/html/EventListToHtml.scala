...
/**
 * Converts an EventModel instance to a HTML formatted string
 */
//noinspection TypeAnnotation
case class EventListToHtml(list: List[EventModel], pubType: String, compName: String) extends HtmlMarkup {
  private val titleStr = s"$pubType Published by $compName"
  private val head = mkHeading(3, titleStr)

  private val body = {
    import scalatags.Text.all._
    div(this.list.map(EventModelToHTML(_, pubType).markup))
  }
...
//noinspection TypeAnnotation
private case class EventModelToHTML(m: EventModel, pubType: String) extends HtmlMarkup {
  ...
  private val table = mkTable(
    List("Min Rate", "Max Rate", "Archive", "Archive Duration", "Archive Rate"),
    List(List(formatRate(m.minRate), formatRate(m.maxRate), yesNo(m.archive), m.archiveDuration,
