...
object Resolver {
  protected lazy val log: Logger = Logger("csw.services.icd.db.Resolver")
...
case class Resolver(allModels: List[IcdModels]) {
  ...
  private def resolveEvent(publishModel: PublishModel, section: String, eventModel: EventModel): EventModel = {
    val tryRefEventModel =
      if (eventModel.ref.isEmpty) {
        ...
        Try(resolveRefEvent(publishModel, section, eventModel.ref))
      }
  ...
  private def resolveRefAttribute(publishModel: PublishModel, eventType: String, ref: String): AttributeModel = {
    ...
    if (maybeAttribute.isEmpty) {
      throw resolverException(s"Invalid ref '$ref': attribute $name not found")
    }
    maybeAttribute.get
  }
  ...
  private def resolveAttribute(
    ...
    if (attributeModel.ref.isEmpty)
      ...
      Try(resolveRefAttribute(publishModel, section, attributeModel.ref)) match {
  ...
  private def resolvePublishModel(ref: String, component: String, publishModel: PublishModel): PublishModel = {
    ...
    if (maybeRefPublishModel.isEmpty)
      throw resolverException(s"Invalid ref '$ref': component $component not found in subsystem ${publishModel.subsystem}")
  ...
  private def resolveRefEvent(publishModel: PublishModel, eventType: String, ref: String): EventModel = {
    ...
    val (component, section, name) = parts.length match {
      ...
      case _ =>
        throw resolverException(s"Invalid ref '$ref': Expected syntax like: componentName/events/eventName")
    }
    ...
    val eventModelList = section match {
      ...
      case _ =>
        throw resolverException(s"Invalid ref '$ref': invalid event type: $section")
    }
    val maybeEventModel = eventModelList.find(_.name == name)
    if (maybeEventModel.isEmpty)
      throw resolverException(s"Invalid ref '$ref': Event $name not found in $component $section")
