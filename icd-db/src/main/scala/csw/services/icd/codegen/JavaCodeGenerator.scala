...
class JavaCodeGenerator(db: IcdDb) {
          ...
          case "Array" =>
            ...
            arrayType match {
              ...
              case _ =>
                ...
                if (isMatrix)
                  s"""public static final Key<MatrixData<$arrayType>> $pNameKey = JKeyType.${keyType(arrayType)}MatrixKey().make("${p.name}"$unitsArg);"""
                else
                  s"""public static final Key<ArrayData<$arrayType>> $pNameKey = JKeyType.${keyType(arrayType)}ArrayKey().make("${p.name}"$unitsArg);"""
            }
          case t =>
  ...
  def generate(subsystemStr: String, maybeComponent: Option[String], sourceFile: File, maybePackage: Option[String]): Unit = {
    ...
    f.close()
    try {
      s"google-java-format -i $sourceFile".!
    } catch {
      case _: Exception => println("Error: Java formatting failed: Make sure you have 'google-java-format' installed (Try: cs install --contrib google-java-format).")
    }
  }
}
