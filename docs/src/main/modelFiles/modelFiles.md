...
Some events might only be published in a diagnostic mode (or are also published in diagnostic mode).
Two fields can indicate this: `diagnosticModes` is a list of the names of diagnostic modes in which the event is published.
If the `diagnosticModeOnly` flag is set to true, then the event is only published in the diagnostic mode or modes.
The diagnostic modes referenced in the event should be defined in the command model.

```
    {
      name = "highVoltage"
      archive = false
      diagnosticModes = [diagnosticsEngr]
...
|diagnosticModes| no| Defines the diagnostic modes that the component can receive.|
...
The `diagnosticModes` block defines the diagnostic modes that a component can receive.
The fields are shown below:

|Field| Required?| Notes|
|----:|:--------:|:-----|
|hint| yes|The hint or name of the diagnostic mode|
|description| yes|A description of the diagnostic mode|
...
Below is an example of a (top level) diagnostic mode block in a command-model.conf file:

```
diagnosticModes = [
  {
    hint = diagnosticsDebug
    description = """
       This diagnostics mode is used to troubleshoot voltage problems.
       The power event is published at 10hz.
       The crateVoltage event is published at 10hz.
    """
  }
  {
    hint = diagnosticsEngr
    description = """
        This diagnostics mode is used to provide additional information when set.
        The power event is published at 20hz.
        The highVoltage event is published at 20hz.
    """
  }
]
```
